...
const uiService = {
    ...
    renderEmployeeTable: (pagedResult) => {
        ...
        // Unpack the data from the API envelope
        const employees = pagedResult.data;

        // Update Pagination Labels
        const startLabel = pagedResult.totalCount === 0 ? 0 : ((pagedResult.page - 1) * pagedResult.pageSize) + 1;
        const endLabel = Math.min(pagedResult.page * pagedResult.pageSize, pagedResult.totalCount);
        $('#showing-count-label').text(`Showing ${startLabel}-${endLabel} of ${pagedResult.totalCount} employees`);
        ...
        employees.forEach(emp => {
            ...
            const tr = `
                <tr class="text-nowrap">
                    ...
                    <td class="align-middle">${emp.joinDate.split('T')[0]}</td>
                    <td class="align-middle"><span class="badge bg-${statusBadge}">${emp.status}</span></td>
                    <td class="align-middle text-nowrap text-start">
                        <div class="d-flex justify-content-start gap-2">
                            <button class="btn btn-sm btn-outline-info btn-view" data-id="${emp.id}"><i class="bi bi-eye"></i></button>
                            <button class="btn btn-sm btn-outline-secondary btn-edit" data-id="${emp.id}"><i class="bi bi-pencil"></i></button>
        ...
        // Render pagination buttons and apply role restrictions
        uiService.renderPaginationBar(pagedResult);
        uiService.applyRoleUI();
    },

    renderPaginationBar: (pagedResult) => {
        const paginationContainer = $('#pagination-bar'); 
        if (!paginationContainer.length) return; 

        const totalPages = Math.ceil(pagedResult.totalCount / pagedResult.pageSize);

        // If there is only 1 page, hide the pagination bar
        if (totalPages <= 1) {
            paginationContainer.empty();
            return;
        }

        // The Wrapper: We use d-flex and justify-content-between to push the 3 blocks apart
        let html = '<nav class="w-100 mt-3 mb-5 d-flex justify-content-between align-items-center">';
        
        // 1. LEFT BLOCK: Previous Button
        html += `
            <ul class="pagination shadow-sm mb-0">
                <li class="page-item ${!pagedResult.hasPrevPage ? 'disabled' : ''}">
                    <a class="page-link" href="#" data-page="${pagedResult.page - 1}">&laquo; Previous</a>
                </li>
            </ul>
        `;

        // 2. MIDDLE BLOCK: Clickable Page Numbers
        ...
        // 3. RIGHT BLOCK: Next Button
        html += `
            <ul class="pagination shadow-sm mb-0">
                <li class="page-item ${!pagedResult.hasNextPage ? 'disabled' : ''}">
                    <a class="page-link" href="#" data-page="${pagedResult.page + 1}">Next &raquo;</a>
                </li>
            </ul>
        `;
    ...
    populateForm: (emp) => {
        ...
        $('#emp-joinDate').val(emp.joinDate.split('T')[0]); // Strip time if API sends it
    ...
    showModal: (type, data = null) => {
        ...
        if (type === 'add') {
            ...
            $('#view-joinDate').text(data.joinDate.split('T')[0]);
    ...
    applyRoleUI: () => {
     if (authService.isAdmin()) {
         $('.btn-edit, .btn-delete, #nav-add-btn, #page-add-btn').removeClass('d-none');
     } else {
         $('.btn-edit, .btn-delete, #nav-add-btn, #page-add-btn').addClass('d-none');
     }
 }
};